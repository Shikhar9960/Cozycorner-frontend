...
export default function Profile() {
  ...
  const handleFileUpload = async (file) => {
    ...
    try {
      ...
      const response = await fetch("https://api.cloudinary.com/v1_1/dls6dxtep/image/upload", {
        method: "POST",
        body: formData,
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`https://cozycorner-backend.onrender.com/api/user/update/${currentUser._id}`, {
        method: 'POST',
        headers: {
  ...
  const handleDeleteUser = async () => {
    try {
      ...
      const res = await fetch(`https://cozycorner-backend.onrender.com/api/user/delete/${currentUser._id}`, {
        method: 'DELETE',
      });
      const data = await res.json();
      if (data.success === false) {
  ...
  const handleSignOut = async () => {
    try {
      ...
      const res = await fetch('https://cozycorner-backend.onrender.com/api/auth/signout');
  ...
  const handleShowListings = async () => {
    try {
      ...
      const res = await fetch(`https://cozycorner-backend.onrender.com/api/user/listings/${currentUser._id}`);
      const data = await res.json();
      if (data.success === false) {
  ...
  const handleListingDelete = async (listingId) => {
    try {
      const res = await fetch(`https://cozycorner-backend.onrender.com/api/listing/delete/${listingId}`, {
        method: 'DELETE',
      });
      const data = await res.json();
      if (data.success === false) {
    ...
    <div className='p-3 max-w-lg mx-auto'>
      ...
      <form onSubmit={handleSubmit} className='flex flex-col gap-4'>
        ...
        <img
          ...
          alt="profile"
          className="h-24 w-24 shadow-lg object-cover cursor-pointer self-center mt-2 outline-4" // Adjusted size
        />
        ...
        <Link
          className='bg-darkpink text-white p-3 rounded-lg uppercase text-center hover:opacity-95'
          to={'/create-listing'}
        >
          Create Listing
        </Link>
      ...
      {userListings && userListings.length > 0 && (
        <div className='flex flex-col gap-4'>
          <h1 className='text-center text-slate-500 mt-7 text-2xl font-semibold'>
            Your Listings
          </h1>
          {userListings.map((listing) => (
            <div
              key={listing._id}
              className='border shadow-xl rounded-lg p-3 flex justify-between items-center gap-4'
            >
